...
pub trait FieldTranscript<F> {
    ...
    fn common_field_element(&mut self, fe: &F) -> Result<(), ProtocolError>;

    fn common_field_elements(&mut self, fes: &[F]) -> Result<(), ProtocolError> {
        fes.iter()
...
pub trait FieldTranscriptRead<F>: FieldTranscript<F> {
    fn read_field_element(&mut self) -> Result<F, ProtocolError>;

    fn read_field_elements(&mut self, n: usize) -> Result<Vec<F>, ProtocolError> {
        (0..n).map(|_| self.read_field_element()).collect()
    }
}

pub trait FieldTranscriptWrite<F>: FieldTranscript<F> {
    fn write_field_element(&mut self, fe: &F) -> Result<(), ProtocolError>;
...
pub trait Transcript<C, F>: FieldTranscript<F> {
    fn common_commitment(&mut self, comm: &C) -> Result<(), ProtocolError>;

    fn common_commitments(&mut self, comms: &[C]) -> Result<(), ProtocolError> {
        comms
            .iter()
...
pub trait TranscriptRead<C, F>: Transcript<C, F> + FieldTranscriptRead<F> {
    fn read_commitment(&mut self) -> Result<C, ProtocolError>;

    fn read_commitments(&mut self, n: usize) -> Result<Vec<C>, ProtocolError> {
...
impl<H: Hash, F: PrimeField, R: io::Read> FieldTranscriptRead<F> for FiatShamirTranscript<H, R> {
    fn read_field_element(&mut self) -> Result<F, ProtocolError> {
        ...
        self.stream
            ...
            .map_err(|err| ProtocolError::Transcript(err.kind(), err.to_string()))?;
        repr.as_mut().reverse();
    ...
    ($($curve:ty),*$(,)?) => {
        $(
            impl<H: Hash, S> Transcript<$curve, <$curve as CurveAffine>::ScalarExt> for FiatShamirTranscript<H, S> {
                fn common_commitment(&mut self, comm: &$curve) -> Result<(), ProtocolError> {
                    let coordinates =
            ...
            {
                fn read_commitment(&mut self) -> Result<$curve, ProtocolError> {
            ...
            {
                fn write_commitment(&mut self, ec_point: &$curve) -> Result<(), ProtocolError> {
                    self.common_commitment(ec_point)?;
...
impl<F: PrimeField, S> Transcript<Output<Keccak256>, F> for Keccak256Transcript<S> {
    fn common_commitment(&mut self, comm: &Output<Keccak256>) -> Result<(), ProtocolError> {
...
impl<F: PrimeField, R: io::Read> TranscriptRead<Output<Keccak256>, F> for Keccak256Transcript<R> {
    fn read_commitment(&mut self) -> Result<Output<Keccak256>, ProtocolError> {
...
impl<F: PrimeField, W: io::Write> TranscriptWrite<Output<Keccak256>, F> for Keccak256Transcript<W> {
    fn write_commitment(&mut self, hash: &Output<Keccak256>) -> Result<(), ProtocolError> {
        self.stream
            .write_all(hash)
            .map_err(|err| ProtocolError::Transcript(err.kind(), err.to_string()))?;
        Ok(())
